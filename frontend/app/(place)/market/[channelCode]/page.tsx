...
import {
  ...
  getMarketBrandAdLogos,
  getMarketHeroBannerConfig
} from '@/lib/market-banner-api'
...
type MarketChannelHubContentProps = {
...
const DEFAULT_BRAND_AD_TITLE =
  '농심 · 오뚜기 · SAMYANG · CJ 제일제당 · 동원F&B · 풀무원 · 롯데'
...
export function MarketChannelHubContent({
  defaultChannelCode
}: MarketChannelHubContentProps) {
  const router =
    useRouter()
  const globalMenuRef =
  ...
  const [heroBanner, setHeroBanner] =
    useState<MarketHeroBanner | null>(null)
  const [heroBanners, setHeroBanners] =
    useState<MarketHeroBanner[]>([])
  ...
  const [brandAdLogos, setBrandAdLogos] =
    useState<MarketBrandAdLogo[]>([])
  const [activeHeroIndex, setActiveHeroIndex] =
    useState(0)
  const [isHeroPaused, setIsHeroPaused] =
    useState(false)
  const sponsorCarouselViewportRef =
    useRef<HTMLDivElement | null>(null)
    ...
    async function loadHeroBanner() {
      ...
      try {
        const [
          ...
          await Promise.all([
            getMarketHeroBannerConfig(channelCode),
            getMarketBrandAdConfig(channelCode),
            getMarketBrandAdLogos(channelCode)
          ])

        if (isMounted) {
          setHeroBanner(nextHeroBanner.banner)
          setHeroBanners(nextHeroBanner.banners)
          setBrandAdConfig(nextBrandAdConfig)
          setBrandAdLogos(nextBrandAdLogos)
        }
      } catch (error) {
        if (isMounted) {
          setHeroBanner(null)
  ...
  const channelInfoName =
    ...
    channel.marketName?.trim() ||
    '상호명 미설정'
  const searchPlaceholderMarketName =
    channelInfoName === '상호명 미설정'
      ? '마켓'
      : channelInfoName
  ...
  const activeHeroBanners =
    useMemo(() => {
      return heroBanners
        .filter((banner) => {
          return banner.displayStatus === 'VISIBLE' && banner.isActive !== 0
        })
        .sort((left, right) => {
  ...
  const currentHeroBanner =
    activeHeroBanners[activeHeroIndex] ?? heroBanner
  const heroBannerImageUrl =
    currentHeroBanner?.imageUrl
      ? buildMediaUrl(currentHeroBanner.imageUrl)
      : ''
  const heroBannerLinkUrl =
    String(currentHeroBanner?.linkUrl ?? '')
      .trim()
  const isHeroBannerLinkExternal =
    heroBannerLinkUrl.startsWith('http://') ||
    heroBannerLinkUrl.startsWith('https://')
  const isHeroBannerLinkInternal =
    heroBannerLinkUrl.startsWith('/')
  const isHeroBannerClickable =
    Boolean(heroBannerLinkUrl) &&
    (isHeroBannerLinkExternal || isHeroBannerLinkInternal)

  function getHeroNavTitle(hero: MarketHeroBanner, fallbackIndex: number) {
    const titleText =
    ...
    return `${fallbackIndex + 1}번 히어로`
  }

  function getHeroNavSubtitle(hero: MarketHeroBanner) {
    const descriptionText =
      String(hero.description ?? '').trim()

    if (descriptionText) {
      return descriptionText
    }

    return '마켓 메인 히어로 이미지'
  }

  function handleMoveHeroSlide(direction: 'prev' | 'next') {
    ...
    setActiveHeroIndex((currentIndex) => {
      ...
      return currentIndex >= activeHeroBanners.length - 1
        ? 0
        : currentIndex + 1
    })
  }

  function handleClickHeroBannerImage() {
    if (!isHeroBannerClickable) {
      return
    }

    if (isHeroBannerLinkExternal) {
      window.open(
        heroBannerLinkUrl,
        '_blank',
        'noopener,noreferrer'
    ...
    router.push(heroBannerLinkUrl)
  }

  const visibleBrandAdLogos =
    useMemo(() => {
      return brandAdLogos
        .filter((logo) => logo.displayStatus === 'VISIBLE' && logo.isActive !== 0)
  ...
  const sponsorLogos =
    useMemo(() => {
      if (visibleBrandAdLogos.length > 0) {
        return visibleBrandAdLogos
          .map((logo) => {
            return {
              key: `brand-logo-${logo.id}-${logo.sortOrder}`,
              imageUrl: logo.imageUrl
                ? buildMediaUrl(logo.imageUrl)
                : '',
              alt: `스폰서 브랜드 로고 ${logo.sortOrder}`
            }
          })
          .filter((item) => Boolean(item.imageUrl))
      }

      return SPONSOR_LOGOS.map((logo, index) => {
        return {
          key: `fallback-brand-logo-${index + 1}`,
          text: logo,
          alt: logo
        }
      })
  ...
  function moveSponsorCarousel(direction: 'prev' | 'next') {
    const viewport =
      sponsorCarouselViewportRef.current
    ...
    const scrollAmount =
      Math.max(180, Math.floor(viewport.clientWidth * 0.6))
  ...
  useEffect(() => {
    ...
    setActiveHeroIndex((currentIndex) => {
      return Math.min(currentIndex, activeHeroBanners.length - 1)
    })
  }, [
  ...
  useEffect(() => {
    if (activeHeroBanners.length <= 1 || isHeroPaused) {
      return
    }
    ...
    <main className={styles.page}>
      <div className={styles.shell}>
        ...
        <section
          className={[
            styles.heroBanner,
            heroBannerImageUrl ? styles.heroBannerWithAdminImage : ''
          ].filter(Boolean).join(' ')}
          aria-label="마켓 프로모션"
          onMouseEnter={() => {
        ...
        >
          {heroBannerImageUrl ? (
            <div
              className={[
                styles.heroAdminImageFrame,
                isHeroBannerClickable ? styles.heroAdminImageFrameClickable : ''
              ].filter(Boolean).join(' ')}
              role={isHeroBannerClickable ? 'button' : undefined}
              tabIndex={isHeroBannerClickable ? 0 : undefined}
              onClick={handleClickHeroBannerImage}
              onKeyDown={(event) => {
                if (!isHeroBannerClickable) {
                  return
                }
                ...
                if (event.key === 'Enter' || event.key === ' ') {
                  event.preventDefault()
                  handleClickHeroBannerImage()
                }
              }}
              aria-label={
                isHeroBannerClickable
                  ? '히어로 이미지 링크 이동'
                  : undefined
              }
            >
              ...
              {activeHeroBanners.length > 1 ? (
                <>
                  <button
                    type="button"
                    className={`${styles.heroSlideButton} ${styles.heroSlideButtonPrev}`}
                    aria-label="이전 히어로 이미지"
                    onClick={() => {
                      handleMoveHeroSlide('prev')
                    }}
                  >
                    ‹
                  </button>
                  <button
                    type="button"
                    className={`${styles.heroSlideButton} ${styles.heroSlideButtonNext}`}
                    aria-label="다음 히어로 이미지"
                    onClick={() => {
                      handleMoveHeroSlide('next')
                    }}
                  >
                    ›
                  </button>
          ...
          <aside className={styles.heroSidePanel}>
            ...
            {activeHeroBanners.length > 0 ? (
              activeHeroBanners.map((hero, index) => {
                const isSelected =
                  index === activeHeroIndex

                return (
                  <button
                    key={`${hero.id}-${hero.sortOrder}`}
                    type="button"
                    className={[
                      styles.heroNavItem,
                      isSelected ? styles.heroNavItemActive : ''
                    ].filter(Boolean).join(' ')}
                    onClick={() => {
                      setActiveHeroIndex(index)
                    }}
                    aria-label={`${index + 1}번 히어로 이미지로 이동`}
                  >
                    <span>
                      {getHeroNavTitle(hero, index)}
                    </span>
                    <b>
                      {getHeroNavSubtitle(hero)}
                    </b>
                  </button>
                )
              })
        ...
        >
          <div className={styles.sponsorCarousel}>
            <button
              type="button"
              className={styles.sponsorNavButton}
              onClick={() => {
                moveSponsorCarousel('prev')
              }}
            ...
            <div
              ref={sponsorCarouselViewportRef}
              className={styles.sponsorCarouselViewport}
            >
              <div className={styles.sponsorCarouselTrack}>
                {sponsorLogos.map((logo) => (
                  <button
                    key={logo.key}
                    type="button"
                    className={styles.sponsorCarouselItem}
                    aria-label={`${logo.alt} 브랜드 로고`}
                  >
                    {'imageUrl' in logo ? (
                      <img
                        src={logo.imageUrl}
                        alt={logo.alt}
                        className={styles.sponsorLogoThumb}
                      />
                    ) : (
                      <span>
                        {logo.text}
