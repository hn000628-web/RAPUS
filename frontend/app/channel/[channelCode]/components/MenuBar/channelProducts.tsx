// FILE : frontend/app/channel/[channelCode]/components/MenuBar/channelProducts.tsx
// ROOT : frontend/app/channel/[channelCode]/components/MenuBar/channelProducts.tsx
// STATUS : MODIFY MODE
// ROLE : PUBLIC CHANNEL PRODUCT / SERVICE / MENU VIEW COMPONENT
// CHANGE SUMMARY :
// - BusinessProducts 而댄룷?뚰듃瑜??ъ슜??怨듦컻 梨꾨꼸酉??꾩슜 ChannelProducts濡??ш?怨?
// - getMe / authApi / business-posts-api 吏곸젒 ?몄텧 ?쒓굅
// - 遺紐?而댄룷?뚰듃?먯꽌 ?꾨떖諛쏆? items / loading / errorMessage 湲곗? ?뚮뜑留?
// - PRODUCT ?곗씠??議댁옱 ??移대뱶 洹몃━?쒕쭔 ?쒖떆
// - loading ?곹깭?먯꽌 ??댄? ?놁씠 ?ㅼ펷?덊넠留??쒖떆
// - error ?곹깭?먯꽌 議고쉶 ?ㅽ뙣 諛뺤뒪留??쒖떆
// - empty ?곹깭?먯꽌 鍮꾩뼱 ?덉쓬 諛뺤뒪留??쒖떆
// - ????대?吏 / 媛寃?/ ?쒕ぉ / ?댁슜 ?붿빟 異쒕젰 ?좎?
...
export type ChannelProductViewItem = {
  id: number
  title: string
  content?: string | null
...
type ChannelProductsProps = {
  items?: ChannelProductViewItem[] | null
  loading?: boolean
...
export default function ChannelProducts({
  ...
  function handleOpenPost(
    ...
    router.push(
      `/channel/${encodeURIComponent(channelCode)}/product/${postId}`
    )
  }
...
/*
VALIDATION:
- 梨꾨꼸酉??꾩슜 ChannelProducts 而댄룷?뚰듃濡?蹂寃??꾨즺
- BusinessProducts ?ㅼ씠諛??쒓굅 ?꾨즺
- getMe / authApi 吏곸젒 ?몄텧 ?쒓굅 ?꾨즺
- getBusinessProductPosts 吏곸젒 ?몄텧 ?쒓굅 ?꾨즺
- 遺紐?props 湲곕컲 items / loading / errorMessage ?뚮뜑留?援ъ“ ?꾨즺
- PRODUCT 移대뱶 洹몃━??異쒕젰 ?좎? ?꾨즺
- loading / error / empty ?곹깭 泥섎━ ?꾨즺
- DB 吏곸젒 ?묎렐 ?놁쓬
- API 吏곸젒 ?몄텧 ?놁쓬
- 愿由ъ옄 鍮꾩쫰?덉뒪 ?꾨줈??而댄룷?뚰듃 ?곹뼢 ?놁쓬
*/
