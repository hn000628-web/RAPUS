// FILE : frontend/app/channel/[channelCode]/order/components/KioskSubMenuContent.tsx
// ROOT : frontend/app/channel/[channelCode]/order/components/KioskSubMenuContent.tsx
// STATUS : CREATE MODE
// ROLE : PUBLIC BUSINESS CHANNEL KIOSK SUB MENU CONTENT COMPONENT
// CHANGE SUMMARY :
// - 키오스크 주문 서브 메뉴 전용 콘텐츠 컴포넌트 신규 생성
// - 메뉴 썸네일 카드 + 단가 / 합계 / 수량 조절 UI 구성
...
const SUB_MENU_ITEMS: KioskMenuItem[] = [
  {
    id: 'egg-roll',
    name: '계란말이',
    description: '식사와 함께 먹기 좋은 사이드형 서브 메뉴입니다.',
    price: 4500,
  ...
  },
  {
    id: 'fish-cake-soup',
    name: '어묵탕',
    description: '따뜻하게 곁들이는 국물형 서브 메뉴입니다.',
    price: 5000,
    thumbnailUrl: buildMenuThumbnailDataUrl(
      '어묵탕',
      '#2563eb',
  ...
  },
  {
    id: 'corn-cheese',
    name: '콘치즈',
    description: '고소한 옥수수와 치즈를 함께 즐기는 메뉴입니다.',
    price: 5500,
    thumbnailUrl: buildMenuThumbnailDataUrl(
      '콘치즈',
      '#f59e0b',
  ...
  },
  {
    id: 'tofu-kimchi',
    name: '두부김치',
    description: '두부와 볶음김치를 함께 제공하는 메뉴입니다.',
    price: 6500,
    thumbnailUrl: buildMenuThumbnailDataUrl(
      '두부김치',
      '#ef4444',
  ...
  },
  {
    id: 'seasoned-seaweed',
    name: '김가루밥',
    description: '가볍게 추가할 수 있는 서브 밥 메뉴입니다.',
    price: 2500,
    thumbnailUrl: buildMenuThumbnailDataUrl(
      '김가루밥',
      '#111827',
...
export default function KioskSubMenuContent({
  channelCode,
  resetSignal,
  onTotalAmountChange
}: Props) {
  ...
  const [selectedMenuId, setSelectedMenuId] =
    useState<string>(SUB_MENU_ITEMS[0]?.id || '')
  ...
  const totalAmount = useMemo(() => {
    return SUB_MENU_ITEMS.reduce((sum, item) => {
      const quantity =
        getMenuQuantity(
          menuQuantities,
          item.id
  ...
  useEffect(() => {
    setMenuQuantities({})
    setSelectedMenuId(SUB_MENU_ITEMS[0]?.id || '')
  }, [
    resetSignal
  ])
  ...
  const IntroUI = (
    <section style={introCardStyle}>
      ...
      <p style={descriptionStyle}>
        채널 코드 {channelCode || '-'} 기준 키오스크 서브 메뉴입니다.
