// FILE : frontend/app/channel/[channelCode]/order/components/KioskDrinkMenuContent.tsx
// ROOT : frontend/app/channel/[channelCode]/order/components/KioskDrinkMenuContent.tsx
// STATUS : CREATE MODE
// ROLE : PUBLIC BUSINESS CHANNEL KIOSK DRINK MENU CONTENT COMPONENT
// CHANGE SUMMARY :
// - 키오스크 주문 음료 메뉴 전용 콘텐츠 컴포넌트 신규 생성
...
const DRINK_MENU_ITEMS: KioskMenuItem[] = [
  {
    id: 'cola',
    name: '콜라',
...
export default function KioskDrinkMenuContent({
  channelCode,
  resetSignal,
  onTotalAmountChange
}: Props) {
  ...
  const [selectedMenuId, setSelectedMenuId] =
    useState<string>(DRINK_MENU_ITEMS[0]?.id || '')
  ...
  const totalAmount = useMemo(() => {
    return DRINK_MENU_ITEMS.reduce((sum, item) => {
      const quantity =
        getMenuQuantity(
          menuQuantities,
          item.id
  ...
  useEffect(() => {
    setMenuQuantities({})
    setSelectedMenuId(DRINK_MENU_ITEMS[0]?.id || '')
  }, [
    resetSignal
  ])
  ...
  const IntroUI = (
    <section style={introCardStyle}>
      ...
      <p style={descriptionStyle}>
        채널 코드 {channelCode || '-'} 기준 키오스크 음료 메뉴입니다.
