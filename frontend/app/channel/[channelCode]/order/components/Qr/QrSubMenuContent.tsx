// FILE : frontend/app/channel/[channelCode]/order/components/Qr/QrSubMenuContent.tsx
// ROOT : frontend/app/channel/[channelCode]/order/components/Qr/QrSubMenuContent.tsx
// STATUS : CREATE MODE
// ROLE : PUBLIC BUSINESS CHANNEL QR SUB MENU CONTENT COMPONENT
// CHANGE SUMMARY :
// - QR코드 주문 서브 메뉴 전용 콘텐츠 컴포넌트 신규 생성
// - 서브 메뉴 썸네일 카드 + 단가 / 합계 / 수량 조절 UI 구성
...
const QR_SUB_MENU_ITEMS: QrMenuItem[] = [
  {
    id: 'egg-roll',
    name: '계란말이',
    description: '식사와 함께 먹기 좋은 서브 메뉴입니다.',
    price: 4500,
...
export default function QrSubMenuContent({
  channelCode,
  resetSignal,
  onTotalAmountChange
}: Props) {
  ...
  const [selectedMenuId, setSelectedMenuId] =
    useState<string>(QR_SUB_MENU_ITEMS[0]?.id || '')
  ...
  const totalAmount = useMemo(() => {
    return QR_SUB_MENU_ITEMS.reduce((sum, item) => {
      const quantity =
        getMenuQuantity(
          menuQuantities,
          item.id
  ...
  useEffect(() => {
    setMenuQuantities({})
    setSelectedMenuId(QR_SUB_MENU_ITEMS[0]?.id || '')
  }, [
    resetSignal
  ])
  ...
  const IntroUI = (
    <section style={introCardStyle}>
      ...
      <p style={descriptionStyle}>
        채널 코드 {channelCode || '-'} 기준 QR코드 주문 서브 메뉴입니다.
