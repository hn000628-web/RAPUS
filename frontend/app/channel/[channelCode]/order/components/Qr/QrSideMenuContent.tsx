// FILE : frontend/app/channel/[channelCode]/order/components/Qr/QrSideMenuContent.tsx
// ROOT : frontend/app/channel/[channelCode]/order/components/Qr/QrSideMenuContent.tsx
// STATUS : CREATE MODE
// ROLE : PUBLIC BUSINESS CHANNEL QR SIDE MENU CONTENT COMPONENT
// CHANGE SUMMARY :
// - QR코드 주문 사이드 메뉴 전용 콘텐츠 컴포넌트 신규 생성
...
const QR_SIDE_MENU_ITEMS: QrMenuItem[] = [
  {
    id: 'rice',
    name: '공기밥',
...
export default function QrSideMenuContent({
  channelCode,
  resetSignal,
  onTotalAmountChange
}: Props) {
  ...
  const [selectedMenuId, setSelectedMenuId] =
    useState<string>(QR_SIDE_MENU_ITEMS[0]?.id || '')
  ...
  const totalAmount = useMemo(() => {
    return QR_SIDE_MENU_ITEMS.reduce((sum, item) => {
      const quantity =
        getMenuQuantity(
          menuQuantities,
          item.id
  ...
  useEffect(() => {
    setMenuQuantities({})
    setSelectedMenuId(QR_SIDE_MENU_ITEMS[0]?.id || '')
  }, [
    resetSignal
  ])
  ...
  const IntroUI = (
    <section style={introCardStyle}>
      ...
      <p style={descriptionStyle}>
        채널 코드 {channelCode || '-'} 기준 QR코드 주문 사이드 메뉴입니다.
