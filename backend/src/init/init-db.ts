...
rebuildLegacyProductCodeTable(
  'pos_products',
  "CASE WHEN productCode IS NOT NULL AND length(productCode) = 12 THEN productCode ELSE 'PR' || substr('0000000000' || id, -10, 10) END"
)

rebuildLegacyProductCodeTable(
  'pos_product_thumbnails',
  "COALESCE((SELECT p.productCode FROM pos_products p WHERE p.id = pos_product_thumbnails.productId), CASE WHEN productCode IS NOT NULL AND length(productCode) = 12 THEN productCode ELSE NULL END)"
)

rebuildLegacyProductCodeTable(
  'pos_product_options',
  "COALESCE((SELECT p.productCode FROM pos_products p WHERE p.id = pos_product_options.productId), CASE WHEN productCode IS NOT NULL AND length(productCode) = 12 THEN productCode ELSE NULL END)"
)

rebuildLegacyProductCodeTable(
  'pos_product_option_values',
  "COALESCE((SELECT o.productCode FROM pos_product_options o WHERE o.id = pos_product_option_values.optionId), CASE WHEN productCode IS NOT NULL AND length(productCode) = 12 THEN productCode ELSE NULL END)"
)
